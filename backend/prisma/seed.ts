...
async function main() {
  ...
  const currentMonth = await prisma.bookClubMonth.upsert({
    ...
    create: {
      ...
      status: MonthStatus.VOTING,
    },
  });
  console.log(`Created current month: ${currentMonthKey} (host: Alice)`);

  // ── Book options ──────────────────────────────────────────────────────────
  const existingBooks = await prisma.bookOption.count({ where: { monthId: currentMonth.id } });
  if (existingBooks === 0) {
  ...
  // ── Date options ──────────────────────────────────────────────────────────
  const existingDates = await prisma.dateOption.count({ where: { monthId: currentMonth.id } });
  if (existingDates === 0) {
    const nextMonth = new Date(now.getFullYear(), now.getMonth() + 1, 1);
    await prisma.dateOption.createMany({
      data: [
        { monthId: currentMonth.id, date: new Date(nextMonth.getFullYear(), nextMonth.getMonth(), 5, 19, 0), label: 'Saturday evening' },
        { monthId: currentMonth.id, date: new Date(nextMonth.getFullYear(), nextMonth.getMonth(), 12, 19, 0), label: 'Saturday evening' },
        { monthId: currentMonth.id, date: new Date(nextMonth.getFullYear(), nextMonth.getMonth(), 19, 19, 0), label: 'Saturday evening' },
      ],
    });
  ...
  const pastMonthKey = `${now.getFullYear()}-${String(now.getMonth()).padStart(2, '0')}` || `${now.getFullYear() - 1}-12`;
  ...
  const existingPastBooks = await prisma.bookOption.findMany({ where: { monthId: pastMonth.id } });
